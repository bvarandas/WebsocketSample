...
namespace Gradual.Spider.ServicoSupervisor.Calculator
{
    public class BmfCalculator
    {
        ...
        public decimal RetornaFatorMultiplicador(string pInstrumento, double vlrAjuste, double vlrUltimoValor)
        {
            ...
            switch (ClassificacaoInstrumento)
            {
                ...
                case DI:
                    {
                        double lRetornoTemp = CalcularTaxaDI(pInstrumento, vlrAjuste, vlrUltimoValor); // TODO [FF]: rever o calculo
        ...
        /*
        public bool CalcularPosicaoBmfComLPAberta(PosClientSymbolInfo pos)
        {
            bool ret = false;
            try
            {
                ...
                decimal lLucroAbertura = 0M;

                decimal lCustodiaAbertura = 0M;
                ...
                if (pos.QtdAbertura != 0)
                {
                    if (pos.Ativo == "OZ1D")
                    {
                        lLucroAbertura = this.CalcularLucroPrejuizoBmf(lSymb, decimal.Parse(pos.QtdAbertura.ToString()));

                        lCustodiaAbertura = this.CalcularCustodiaAberturaBmf(lSymb, decimal.Parse(pos.QtdAbertura.ToString()));
                    }
                    else
                    {
                        lLucroAbertura = this.CalcularLucroPrejuizoBmf(lSymb, int.Parse(pos.QtdAbertura.ToString()));

                        lCustodiaAbertura = this.CalcularCustodiaAberturaBmf(lSymb, int.Parse(pos.QtdAbertura.ToString()));
                    }
                }
                ...
                pos.FinancAbertura += lCustodiaAbertura;

                pos.FinancNet = lLucroAbertura;

                pos.LucroPrej = sell + buy +(lLucroAbertura);

                pos.LucroPrejuizoSemAbertura = sell + buy;
        ...
        public bool CalcularPosicaoBmf(PosClientSymbolInfo pos)
        {
            ...
            try
            {
                ...
                decimal fatorMultiplicador = this.RetornaFatorMultiplicador(pos.Ativo,  Convert.ToDouble(vlrUltimo), Convert.ToDouble(vlrUltimo));
                ...
                if (pos.QtdAbertura != 0)
                {
                    if (pos.Ativo == "OZ1D")
                    {
                        ...
                        lCustodiaAbertura = this.CalcularCustodiaAberturaBmf(lSymb, decimal.Parse(pos.QtdAbertura.ToString()));
                         * */
                        lLucroAbertura = (lSymb.VlrUltima - lSymb.VlrAjuste) * pos.QtdAbertura;

                        lCustodiaAbertura = (lSymb.VlrUltima - lSymb.VlrAjuste) * pos.QtdAbertura;
                    }
                    else
                    {
                        lLucroAbertura = this.CalcularLucroPrejuizoBmf(lSymb, int.Parse(pos.QtdAbertura.ToString()));
                ...
                string ClassificacaoInstrumento = lSymb.Instrumento.Substring(0, 3);
                ...
                {
                    buy  = (vlrUltimo - pos.PcMedC) * pos.QtdExecC * fatorMultiplicador*(-1);

                    sell = (pos.PcMedV - vlrUltimo) * pos.QtdExecV * fatorMultiplicador;
                }
                else
                {
                    buy  = (vlrUltimo - pos.PcMedC) * pos.QtdExecC * fatorMultiplicador;
        ...
        public decimal CalcularCustodiaAberturaBmf(SymbolInfo symb, int qtd)
        {
            ...
            try
            {
                decimal fatorMultiplicador = this.RetornaFatorMultiplicador(symb.Instrumento, Convert.ToDouble(symb.VlrAjuste), Convert.ToDouble(symb.VlrUltima));

                string ClassificacaoInstrumento = symb.Instrumento.Substring(0, 3);

                if (DI == ClassificacaoInstrumento)
                {
                    ret = qtd * fatorMultiplicador;
                }
                else if (ClassificacaoInstrumento == "OZ1" ||
                        ClassificacaoInstrumento == "OZ2" ||
                        ClassificacaoInstrumento == "OZ3")
                {
                    ret = (symb.VlrUltima  * qtd);
                }
                else
                {
        ...
        public decimal CalcularCustodiaAberturaBmf(SymbolInfo symb, decimal qtd)
        {
            decimal ret = decimal.Zero;
            try
            {
        ...
        public decimal CalcularLucroPrejuizoBmf(SymbolInfo symb, decimal qtd)
        {
            decimal ret = decimal.Zero;
        ...
        public decimal CalcularLucroPrejuizoBmf(SymbolInfo symb, int qtd)
        {
            decimal ret = decimal.Zero;
            try
            {
        ...
        public decimal CalcularLucroPrejuizoBmfTotais(SymbolInfo symb, decimal qtd, decimal Preco)
        {
            decimal ret = decimal.Zero;
            try
            {
                decimal fatorMultiplicador = this.RetornaFatorMultiplicador(symb.Instrumento, Convert.ToDouble(Preco), Convert.ToDouble(symb.VlrUltima));
                ...
                //ret = Preco * qtd * fatorMultiplicador;
            }
            catch (Exception ex)
            {
                logger.Error("Problemas no calculo de resultado financeiro bmf: " + ex.Message, ex);
        ...
        public decimal CalcularLPSubtotal(SymbolInfo symb, decimal qtd, decimal DiferencialPontos, decimal PrecoOperado)
        {
            decimal ret = decimal.Zero;
            try
            {
                decimal fatorMultiplicador = this.RetornaFatorMultiplicador(symb.Instrumento, Convert.ToDouble(PrecoOperado), Convert.ToDouble(symb.VlrUltima));
                ...
                //ret = DiferencialPontos * qtd * fatorMultiplicador;
            }
            catch (Exception ex)
            {
                logger.Error("Problemas no calculo de resultado financeiro bmf: " + ex.Message, ex);
