    ...
    public class AcSpiderCache
    {
        ...
        void _client_OnSpiderOrderInfoReceived(object sender, int clientNumber, Socket clientSocket, SpiderOrderInfo args)
        {
            try
            {
                ...
                {
                    logger.DebugFormat("OrderID[{0}] Account[{1}] Symbol[{2}] Side[{3}] Status[{4}] Detail[{5}] OrderQty[{6}] OrderQtyRemaining[{7}] CumQty[{8}] Price [{9}] Horario [{10}] IsSnapshot[{11}]",
                        args.OrderID, args.Account, args.Symbol, args.Side, args.OrdStatus, args.Details.Count, args.OrderQty, args.OrderQtyRemaining, args.CumQty, args.Price, args.TransactTime.ToString("HH:mm"), args.IsSnapshot.ToString() );
                }
