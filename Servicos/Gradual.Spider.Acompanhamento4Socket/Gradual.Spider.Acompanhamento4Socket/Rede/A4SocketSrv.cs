        ...
        void _sckServer_OnClientConnected(object sender, ClientConnectedEventArgs args)
        {
            try
            {
                lock (_dicConn)
                {
                    ...
                    aux.ClientSocket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.SendTimeout, 2000);
                    ...
                    if (ts.TotalMilliseconds > 30000 && _dicConn.Count > 0)
                    {
                        ...
                        foreach (int clientNumber in toDelete)
                        {
                            try
                            {
                                // Send operations will time-out if confirmation 
                                // is not received within 1000 milliseconds.
                                _dicConn[clientNumber].ConnectionInfo.ClientSocket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.SendTimeout, 2000);

                                // The socket will linger for 00 seconds after Socket.Close is called.
                                LingerOption lingerOption = new LingerOption(true, 0);

                                _dicConn[clientNumber].ConnectionInfo.ClientSocket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.Linger, lingerOption);
                                _dicConn[clientNumber].ConnectionInfo.ClientSocket.Shutdown(SocketShutdown.Both);
