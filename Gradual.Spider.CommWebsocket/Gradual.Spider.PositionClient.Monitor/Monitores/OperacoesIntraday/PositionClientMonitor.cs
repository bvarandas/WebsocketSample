    ...
    public class PositionClientMonitor : IServicoControlavel, IServicoWebSocket, IServicoPositionClientMonitor
    {
        ...
        public void IniciarServico()
        {
            try
            {
                ...
                _Logger.Info("[Iniciando thread de fila de mensagens de position client.]");
        ...
        public void PararServico()
        {
            try
            {
                ...
                if (_ThreadQueueSendPositionClient != null && _ThreadQueueSendPositionClient.IsAlive)
                {
                    _Logger.Info("Aguardando finalizar _ThreadQueueSendPositionClient");

                    _ThreadQueueSendPositionClient.Join(500);
                    if (_ThreadQueueSendPositionClient.IsAlive)
                    {
                        try
                        {
                            _ThreadQueueSendPositionClient.Abort();
                            _ThreadQueueSendPositionClient = null;
                        }
                        catch { }
                    }
                }
        ...
        public bool TrataFiltroEfetuado(WebSocketSession pSession, PosClientSymbolInfo pInfo)
        {
            try
            {
                ...
                if (lFiltro.OpcaoParametroIntradayNetPositivo)
                {
                    if (pInfo.NetExec < 0)
                    { return false; }
                }
        ...
        public void SendMessageMemoryPositionClientToQueue(object sender, MessagePositionClientArgs e)
        {
            try
            {
                ...
                IEnumerable<PosClientSymbolInfo> lPosicaoEnumerable =  from a in lListaVerifica select a;

                if (e.Session.lOperacoesIntradayRequest.CodigoCliente != 0)
                {
                    lPosicaoEnumerable = from a in lListaVerifica where a.Account == e.Session.lOperacoesIntradayRequest.CodigoCliente select a;
                ...
                foreach (PosClientSymbolInfo lPosicao in lPosicaoEnumerable.ToList())
                {
        ...
        private void QueueSendMessagePositionClient()
        {
            ...
            try
            {
                while (_KeepRunning)
                {
                    ...
                    if (_QueueSessionMessagesPositionClients.TryDequeue(out lSessionMessage))
                    {
