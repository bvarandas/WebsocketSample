    ...
    public class PositionClientSocketRiscoResumido : IDisposable
    {
        ...
        public void StartClientConnect(List<int> pSubscriptions)
        {
            try
            {
                ...
                //catch (Exception ex)
                //{
                //    _Logger.Error("Não conseguiu conectar no Serviço Supervisor Risco: " + ex.Message, ex);
                //}
                //_ClientSocket.AddHandler<SymbolListSyncMsg>(new ProtoObjectReceivedHandler<SymbolListSyncMsg>(OnSymbolListSync));
        ...
        private void OnConsolidatedRiskSync(object sender, int clientNumber, Socket clientSock, ConsolidatedRiskSyncMsg args)
        {
                ...
                switch (args.SyncAction)
                {
                    ...
                    case SyncMsgAction.UPDATE:
                        ...
                        foreach (KeyValuePair<int, ConsolidatedRiskInfo> item in args.ConsolidatedRisk)
                        {
                            ...
                            _Logger.InfoFormat("Recebeu Consolidated Risk do Cliente: [{0}] PL Bovespa: [{1}] PL Bmf:[{2}] Total Custodia: [{3}] Total CC: [{4}] Percentual Atingido: [{5}] Horário Calculo [{6}]",
                                item.Key, 
                                item.Value.PLBovespa, 
                                item.Value.PLBmf,
                                item.Value.TotalCustodiaAbertura,
                                item.Value.TotalContaCorrenteAbertura,
                                item.Value.TotalPercentualAtingido,
                                item.Value.HorarioCalculo.ToString("HH:mm"));
                        }
            ...
            {
                _Logger.Error("Erro encontrado no disparo do evento OnConsolidatedRiskSync", ex);
            }
        }
        ...
        public void MonitorSonda()
        {
            ...
            while (bKeepRuning)
            {
                try
                {
                    ...
                    {
                        _Logger.Info("Sonda de tentativa de conexão no serviço de Supervisor Risco");
