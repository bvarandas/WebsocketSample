...
function PositionClient_Risco_Resumido_Grid() {
    gGridRiscoResumido =
    {
      ...
      , autowidth: false
        //, shrinkToFit: false
      , colModel: [
                        { label: "Remover" ,        name: "Remover",                jsonmap: "Remover",             index: "Remover",               width: 25, align: "center", sortable: true, sorttype: 'int'}
                      , { label: "",                name: "Semaforo",               jsonmap: "Semaforo",            index: "Semaforo",              width: 25, align: "center", sortable: true, sorttype: 'number' }
                      , { label: "Cliente",         name: "CodigoCliente",          jsonmap: "CodigoCliente",       index: "CodigoCliente",         width: 55, align: "center", sortable: true, sorttype: 'number' }
                      , { label: "Nome Cliente",    name: "NomeCliente",            jsonmap: "NomeCliente",         index: "NomeCliente",           width: 75, align: "left",   sortable: true, sorttype: 'number' }
                      , { label: "R$ Cust Abert",   name: "CustodiaAbertura",       jsonmap: "CustodiaAbertura",    index: "CustodiaAbertura",      width: 65, align: "right" , sortable: true ,formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: ".", decimalPlaces: 2, defaultValue: '0.00',    prefix: 'R$ '}, sorttype: 'number'}
                      , { label: "R$ CC Abert",     name: "CCAbertura",             jsonmap: "CCAbertura",          index: "CCAbertura",            width: 55, align: "right" , sortable: true ,formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: ".", decimalPlaces: 2, defaultValue: '0.00',    prefix: 'R$ '}, sorttype: 'number'}
                      , { label: "R$ Garantias",    name: "Garantias",              jsonmap: "Garantias",           index: "Garantias",             width: 75, align: "right" , sortable: true ,formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: ".", decimalPlaces: 2, defaultValue: '0.00',    prefix: 'R$ '}, sorttype: 'number'}
                      , { label: "R$ Produtos",     name: "Produtos",               jsonmap: "Produtos",            index: "Produtos",              width: 75, align: "right" , sortable: true ,formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: ".", decimalPlaces: 2, defaultValue: '0.00',    prefix: 'R$ '}, sorttype: 'number'}
                      , { label: "Total Abertura",  name: "TotalAbertura",          jsonmap: "TotalAbertura",       index: "TotalAbertura",         width: 75, align: "right" , sortable: true ,formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: ".", decimalPlaces: 2, defaultValue: '0.00',    prefix: 'R$ '}, sorttype: 'number'}
                      , { label: "P&L Bovespa",     name: "PLBovespa",              jsonmap: "PLBovespa",           index: "PLBovespa",             width: 75, align: "right" , sortable: true ,formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: ".", decimalPlaces: 2, defaultValue: '0.00',    prefix: 'R$ '}, sorttype: 'number'}
                      , { label: "P&L Bmf",         name: "PLBmf",                  jsonmap: "PLBmf",               index: "PLBmf",                 width: 75, align: "right" , sortable: true ,formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: ".", decimalPlaces: 2, defaultValue: '0.00',    prefix: 'R$ '}, sorttype: 'number'}
                      , { label: "P&L Total",       name: "PLTotal",                jsonmap: "PLTotal",             index: "PLTotal",               width: 75, align: "right" , sortable: true ,formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: ".", decimalPlaces: 2, defaultValue: '0.00',    prefix: 'R$ '}, sorttype: 'number'}
                      , { label: "R$ SFP",          name: "SFP",                    jsonmap: "SFP",                 index: "SFP",                   width: 75, align: "right" , sortable: true ,formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: ".", decimalPlaces: 2, defaultValue: '0.00',    prefix: 'R$ '}, sorttype: 'number'}
                      , { label: "% Atingido",      name: "PercAtingido",           jsonmap: "PercAtingido",        index: "PercAtingido",          width: 75, align: "center", sortable: true ,formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: ".", decimalPlaces: 2, defaultValue: '0.00',  suffix: ' %' }, sorttype: 'number'}
      ]
      , height: 'auto'
        //, width: '100%'
        , width: '1130'
      ...
      , sortable: true
      , beforeSelectRow: BeforeSelectRowEvent
    };
}

function BeforeSelectRowEvent(rowid, e)
{
    var lGrid = $("#jqGrid_RiscoResumido");

    var lCodigoCliente = lGrid.jqGrid('getCell', rowid, 'CodigoCliente');

    if ($.inArray(lCodigoCliente.toString(), gListaRowsSelecionadas) > -1) {

        gListaRowsSelecionadas.splice($.inArray(lCodigoCliente.toString(), gListaRowsSelecionadas), 1);

        //$.grep(gListaRowsSelecionadas, function (value) {
        //    return value != lCodigoCliente.toString();
        //});
    } else {

        gListaRowsSelecionadas.push(lCodigoCliente);
    }
    ...
    {
        var lInitialRowSelect = lGrid.jqGrid('getGridParam', 'selrow');

        lGrid.jqGrid('resetSelection');

        var lCurrentSelectIndex = lGrid.jqGrid('getInd', rowid);

        var lInitialSelectIndex = lGrid.jqGrid('getInd', lInitialRowSelect);

        var lStartID = "";

        var lEndID = "";

        if (lCurrentSelectIndex > lInitialSelectIndex) 
        {
            lStartID = lInitialRowSelect;
            lEndID = rowid;
        }
        else {
            lStartID = rowid;
            lEndID = lInitialRowSelect;
        }

        var lShouldSelectRow = false;

        $.each(lGrid.getDataIDs(), function (_, id)
        {
            if ((lShouldSelectRow = id == lStartID || lShouldSelectRow))
            {
                lGrid.jqGrid('setSelection', id, false);
            }
            return id != lEndID;
        });
...
function SetColorRiscoResumidoInsertRow(rowid, pData)
{
    ...
    if (pData.PLTotal < 0) {
        lGrid.jqGrid('setCell', rowid, 'PLTotal', '', { 'background-color': lColorRed, 'color': lColorWhite });
    }
    ...
    lRow.children("td").eq(1).html("<button class=\"btn btn-danger btn-xs btn-block\" id=\"Risco_Resumido_Cliente_" + pData.CodigoCliente+
                                   "\" onclick=\"return RiscoResumido_Excluir_Cliente_Click(this, " + pData.CodigoCliente + ", " + rowid + ");\" alt=\"Remover Cliente\" >[X]</button>");

    if ($.inArray(pData.CodigoCliente.toString(), gListaRowsSelecionadas) > -1)
    {
...
function RiscoResumido_Excluir_Cliente_Click(pSender, CodigoCliente, rowid) {
    ...
    var lRowIds = lGrid.jqGrid('getDataIDs');

    if (gRiscoResumidoListaClientes.length == 0 || lRowIds.length == 0) {
...
function RiscoResumido_RemoverFiltros_PrejuizoAtingido_Click(pSender) {

    $(pSender).closest("fieldset").find(':checkbox').prop('checked', false);
...
function RiscoResumido_RemoverFiltros_SFPAtingido_Click(pSender) {
...
function RiscoResumido_Remover_Todos_Filtros_Click(pSender)
{
    ...
    $("#pnlRiscoResumido :input").filter(':text').val("");

    $("#pnlRiscoResumido :input").filter(':checkbox').prop('checked', false);
...
function PreencheMockRiscoResumido(pObjeto) 
{
    lMock_Risco_Resumido.rows.push({
        ...
        NomeCliente     : pObjeto.NomeCliente,
...
function RiscoResumido_Bind_Event_Click() {

    $('#pnlRiscoResumido :checkbox').click(
